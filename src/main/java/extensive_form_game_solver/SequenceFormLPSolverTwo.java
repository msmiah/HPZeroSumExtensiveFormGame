...
public class SequenceFormLPSolverTwo<E> extends ZeroSumGameSolver {
    Game game;
    ...
    public SequenceFormLPSolverTwo(Game game, int playerToSolveFor) {
        this(game, playerToSolveFor, 1e-6);
    }

    public SequenceFormLPSolverTwo(Game game, int playerToSolveFor, double tol) {
        super(game);
        this.game = game;
        try {
            cplex = new IloCplex();
...
//            cplex.setParam(IloCplex.IntParam.NetDisplay, 0);
					...
					if (node.getPlayer() == playerToSolveFor) {
						...
						if (null != action) {
							if(node.getPlayer() == playerNotToSolveFor) {
								int realVal = node.getInformationSet() >> 2;
        						realVal++;
                                //System.out.println("Real val :" + realVal);
    ...
    private void CreateDualVariablesAndConstraints() throws IloException {
        ...
        for (int i = 0; i < numVars; i++) { names[i] = "Y" + i;}
          ...
          this.dualVars = cplex.numVarArray(numVars, 0, 1, names);
    ...
    private void InitializePrimalPayoffMatrixRecursive(int currentNodeId,int primalSequence, int dualSequence, double natureProbability) throws IloException{
        ...
        if (node.isLeaf()) {
            int valueMultiplier = playerToSolveFor == 1? -1 : 1;
           // System.out.println("Current node : " + currentNodeId + "Primal seq : " + primalSequence + " Dual seq " + dualSequence);
            double leafValue = valueMultiplier * node.getValue();
           // System.out.println("leaf node val :" + node.getValue());
    ...
    private void CreatePrimalConstraintForSequence(int sequenceId) throws IloException{
        ...
        int val = realValueBySequenceId[sequenceId] >> 2;
        System.out.println("exp for dual : " + lhs + "val : " + val );
        dualConstraints.put(sequenceId, cplex.addGe(lhs, val+1 , "Dual"+sequenceId));
    }
